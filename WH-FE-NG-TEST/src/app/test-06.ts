...
At https://wallethub.com <b>bolded text</b>`
    ...
    toUrl(input: string) {
        ...
        return input.replace(hrefRegex, '<a href="$1" target="_blank">$1</a>')
    }

    formatUrl(input: string) {
        return this.toUrl(input.replace(/<\/?[^>]+(>|$)/g, ""));
     }

     textFieldChange = ($event: any) => {
        this.review_content = this.formatUrl($event.target.value);
     }

    ngOnInit() {
        this.review_content = this.formatUrl(this.review_input);
    }
